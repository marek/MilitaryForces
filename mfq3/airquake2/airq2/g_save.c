...
void InitGame (void)
{
	...
	// +BD Airquake2 cvars and globals
	axistest = gi.cvar( "axistest", "0", 0 );
	maxdrones = gi.cvar( "maxdrones", "1", CVAR_ARCHIVE|CVAR_SERVERINFO );
	eastdrones = gi.cvar( "eastdrones", "1", CVAR_ARCHIVE|CVAR_SERVERINFO );
	weaponinfo = gi.cvar( "weaponinfo", "0", 0 );
	teamplay = gi.cvar( "teamplay", "0", CVAR_LATCH|CVAR_SERVERINFO );
	teamname1 = gi.cvar( "teamname1", "NATO", CVAR_ARCHIVE|CVAR_SERVERINFO );
	teamname2 = gi.cvar( "teamname2", "Warsaw Pact", CVAR_ARCHIVE|CVAR_SERVERINFO );
	rockettrail = gi.cvar( "rockettrail", "1", CVAR_ARCHIVE|CVAR_SERVERINFO );
	attacklanded = gi.cvar( "attacklanded", "1", CVAR_ARCHIVE|CVAR_SERVERINFO );
...
//	fov = gi.cvar( "fov", "90", CVAR_NOSET|CVAR_USERINFO );
	...
	// as long as there is no SP disable it!
	...
	if( teamplay->value == 1 )
	{
		for( loop = 0; loop < 2; loop++ )
		{
			...
			team[loop].drones = 0;
			team[loop].score = 0;
			strcpy( team[loop].name, (loop < 1 ? teamname1->string : teamname2->string ) );
		}
	}
	drone_spawn_time = level.time + 2;
	drone_side = 0;
