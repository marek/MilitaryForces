/*
 * $Id: ui_qmenu.c,v 1.1 2001-11-15 21:35:14 thebjoern Exp $
*/
...
vec4_t color_red		    = {0.86f, 0.00f, 0.17f, 1.00f};
vec4_t color_dim		    = {0.00f, 0.00f, 0.00f, 0.25f};
// MFQ3 colors
vec4_t color_beige			= {0.79f, 0.69f, 0.50f, 1.00f};
vec4_t color_beige_trans	= {0.79f, 0.69f, 0.50f, 0.50f};
//vec4_t color_darkred		= {0.50f, 0.00f, 0.00f, 1.00f};
vec4_t color_darkred	    = {0.76f, 0.00f, 0.17f, 1.00f};
//vec4_t color_grey			= {0.56f, 0.65f, 0.65f, 1.00f};
vec4_t color_grey	        = {0.86f, 0.00f, 0.17f, 1.00f};
vec4_t color_lightgrey		= {0.76f, 0.85f, 0.85f, 1.00f};
vec4_t color_green			= {0.00f, 0.90f, 0.00f, 1.00f};
...
static void Text_Draw( menutext_s *t )
{
	...
	// mfq3 data align (assumes that there is a string)
	if( t->style & UI_DATAALIGN ) {
		char data[32];
		strcpy(data,t->string);
		UI_DrawString( x, y, buff, t->style, color );
		UI_DrawString( x+DATAALIGN_X, y, data, t->style, color );
	}
	else {	// normal
		if (t->string)
...
=================
*/
void Bitmap_Draw_MFQ3( menubitmap_s *b )
{
	float	x;
	float	y;
	float	w;
...
//				UI_DrawHandlePic( x, y, w, h, b->shader );
			qhandle_t shader = trap_R_RegisterShaderNoMip( "menu/art/active" );
			UI_DrawHandlePic( x, y-7, 157, 37, shader );
		}
		UI_DrawHandlePic( x, y, w, h, b->shader );
	}
}
...
static void RadioButton_Draw( menuradiobutton_s *rb )
{
	...
	{
		color = color_beige;
		style = UI_LEFT|UI_SMALLFONT;//|UI_PULSE
	}
	else
	{
		color = color_grey;
	...
	{
		// draw cursor
		UI_FillRect( rb->generic.left, rb->generic.top, rb->generic.right-rb->generic.left+1, rb->generic.bottom-rb->generic.top+1, color_darkred ); 
...
static void Slider_Draw( menuslider_s *s ) {
	...
	left = s->generic.left;
	top = s->generic.top;
	width = s->generic.right - left;
	height = s->generic.bottom - top;
	...
	if( s->generic.flags & QMF_GRAYED ) {
		...
		UI_FillRect( left, top, width, height, color_darkred );
		color  = color_beige;
	...
	for (i=0; i<menu->nitems; i++)
	{
		...
		else 
		{
			...
			{	
				...
				case MTYPE_BITMAP_MFQ3:
					Bitmap_Draw_MFQ3( (menubitmap_s*)itemptr );
					break;
