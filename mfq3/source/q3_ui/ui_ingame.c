/*
 * $Id: ui_ingame.c,v 1.1 2001-11-15 21:35:14 thebjoern Exp $
*/
...
#define INGAME_LOGO						"menu/art/ingame"
...
===============
*/
static void InGameMenu_Draw( void ) {
	UI_DrawNamedPic( 0, 0, 640, 480, INGAME_LOGO );
...
void InGame_MenuInit( void ) {
	...
	s_ingame.menu.draw = InGameMenu_Draw;
	...
	if( UI_teams != TEAM_SPECTATOR ) {
		y += INGAME_MENU_VERTICAL_SPACING;
		s_ingame.vehicle.generic.type		= MTYPE_PTEXT;
		s_ingame.vehicle.generic.flags		= QMF_CENTER_JUSTIFY|QMF_PULSEIFFOCUS;
		s_ingame.vehicle.generic.x			= 320;
		s_ingame.vehicle.generic.y			= y;
		s_ingame.vehicle.generic.id			= ID_VEHICLE;
		s_ingame.vehicle.generic.callback	= InGame_Event; 
		s_ingame.vehicle.string				= "VEHICLE";
		s_ingame.vehicle.color				= color_red;
		s_ingame.vehicle.style				= UI_CENTER|UI_SMALLFONT;
	}
...
//	}
	...
	if( UI_teams != TEAM_SPECTATOR ) {
		Menu_AddItem( &s_ingame.menu, &s_ingame.vehicle );
	}
...
void InGame_Cache( void ) {
	trap_R_RegisterShaderNoMip( INGAME_LOGO );
}
...
void UI_InGameMenu( void ) {
	...
	trap_GetClientState( &cs );
	trap_GetConfigString( CS_PLAYERS + cs.clientNum, info, MAX_INFO_STRING );
	UI_teams = atoi( Info_ValueForKey( info, "t" ) );
