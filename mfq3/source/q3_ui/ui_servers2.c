/*
 * $Id: ui_servers2.c,v 1.1 2001-11-15 21:35:14 thebjoern Exp $
*/
...
#define ART_CREATE				"menu/art/create"
#define ART_CREATE_A			"menu/art/create_a"
#define ART_SPECIFY				"menu/art/specify"
#define ART_SPECIFY_A			"menu/art/specify_a"
#define ART_REFRESH				"menu/art/refresh"
#define ART_REFRESH_A			"menu/art/refresh_a"
#define ART_CONNECT				"menu/art/fight"
#define ART_CONNECT_A			"menu/art/fight_a"
...
static void ArenaServers_Insert( char* adrstr, char* info, int pingtime )
{
	...
	if( Q_stricmp(Info_ValueForKey( info, "game"), "mfq3") != 0 ) {
		// not mfq3
		return;
	}
...
static void ArenaServers_MenuInit( void ) {
	...
	g_arenaservers.up.generic.name				= ART_ARROW_UP;
	...
	g_arenaservers.up.width						= 25;
	g_arenaservers.up.height					= 66;
	g_arenaservers.up.focuspic					= ART_ARROW_UP_A;

	g_arenaservers.down.generic.type			= MTYPE_BITMAP;
	g_arenaservers.down.generic.name			= ART_ARROW_DOWN;
	...
	g_arenaservers.down.width					= 25;
	g_arenaservers.down.height					= 66;
	g_arenaservers.down.focuspic				= ART_ARROW_DOWN_A;
	...
	g_arenaservers.statusbar.color	        = color_red;//text_color_normal;
	...
	g_arenaservers.back.generic.name		= ART_BACK;
	...
	g_arenaservers.back.generic.y			= 425;
	g_arenaservers.back.width				= 96;
	g_arenaservers.back.height				= 32;
	g_arenaservers.back.focuspic			= ART_BACK_A;

	g_arenaservers.specify.generic.type	    = MTYPE_BITMAP;
	g_arenaservers.specify.generic.name		= ART_SPECIFY;
	...
	g_arenaservers.specify.generic.x		= 144;
	g_arenaservers.specify.generic.y		= 425;
	g_arenaservers.specify.width  		    = 96;
	g_arenaservers.specify.height  		    = 32;
	g_arenaservers.specify.focuspic         = ART_SPECIFY_A;

	g_arenaservers.refresh.generic.type		= MTYPE_BITMAP;
	g_arenaservers.refresh.generic.name		= ART_REFRESH;
	...
	g_arenaservers.refresh.generic.x		= 272;
	g_arenaservers.refresh.generic.y		= 425;
	g_arenaservers.refresh.width			= 96;
	g_arenaservers.refresh.height			= 32;
	g_arenaservers.refresh.focuspic			= ART_REFRESH_A;

	g_arenaservers.create.generic.type		= MTYPE_BITMAP;
	g_arenaservers.create.generic.name		= ART_CREATE;
	...
	g_arenaservers.create.generic.x			= 400;
	g_arenaservers.create.generic.y			= 425;
	g_arenaservers.create.width				= 96;
	g_arenaservers.create.height			= 32;
	g_arenaservers.create.focuspic			= ART_CREATE_A;

	g_arenaservers.go.generic.type			= MTYPE_BITMAP;
	g_arenaservers.go.generic.name			= ART_CONNECT;
	...
	g_arenaservers.go.generic.x				= 522;
	g_arenaservers.go.generic.y				= 425;
	g_arenaservers.go.width					= 96;
	g_arenaservers.go.height				= 32;
	g_arenaservers.go.focuspic				= ART_CONNECT_A;
...
void ArenaServers_Cache( void ) {
	...
	trap_R_RegisterShaderNoMip( ART_CREATE );
	trap_R_RegisterShaderNoMip( ART_CREATE_A );
	trap_R_RegisterShaderNoMip( ART_SPECIFY );
	trap_R_RegisterShaderNoMip( ART_SPECIFY_A );
	trap_R_RegisterShaderNoMip( ART_REFRESH );
	trap_R_RegisterShaderNoMip( ART_REFRESH_A );
	trap_R_RegisterShaderNoMip( ART_CONNECT );
	trap_R_RegisterShaderNoMip( ART_CONNECT_A );
