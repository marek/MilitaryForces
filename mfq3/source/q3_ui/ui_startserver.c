/*
 * $Id: ui_startserver.c,v 1.1 2001-11-15 21:35:14 thebjoern Exp $
*/
...
#define GAMESERVER_LOGO				"menu/art/background_sub"
#define GAMESERVER_BACK				"menu/art/back"
#define GAMESERVER_BACK_A			"menu/art/back_a"
#define GAMESERVER_NEXT				"menu/art/next"
#define GAMESERVER_NEXT_A			"menu/art/next_a"
#define GAMESERVER_SELECT			"menu/art/maps_select"
#define GAMESERVER_SELECTED			"menu/art/maps_selected"
#define GAMESERVER_FIGHT			"menu/art/fight"
#define GAMESERVER_FIGHT_A			"menu/art/fight_a"
#define GAMESERVER_UNKNOWNMAP		"menu/art/unknownmap"
#define GAMESERVER_ARROW_LEFT		"menu/art/arrow_left"
#define GAMESERVER_ARROW_LEFT_A		"menu/art/arrow_left_a"
#define GAMESERVER_ARROW_RIGHT		"menu/art/arrow_right"
#define GAMESERVER_ARROW_RIGHT_A	"menu/art/arrow_right_a"
...
static void StartServer_LevelshotDraw( void *self ) {
	...
	UI_DrawString( x, y, s_startserver.maplist[n], UI_CENTER|UI_SMALLFONT, color_grey );
...
{
	UI_DrawNamedPic( 0, 0, 640, 480, GAMESERVER_LOGO );
...
static void StartServer_MenuInit( void ) {
	...
	s_startserver.prevpage.generic.name	    = GAMESERVER_ARROW_LEFT;
	...
	s_startserver.prevpage.generic.y		= 403;
	s_startserver.prevpage.width  		    = 66;
	s_startserver.prevpage.height  		    = 25;
	s_startserver.prevpage.focuspic         = GAMESERVER_ARROW_LEFT_A;

	s_startserver.nextpage.generic.type	    = MTYPE_BITMAP;
	s_startserver.nextpage.generic.name	    = GAMESERVER_ARROW_RIGHT;
	...
	s_startserver.nextpage.generic.y		= 403;
	s_startserver.nextpage.width  		    = 66;
	s_startserver.nextpage.height  		    = 25;
	s_startserver.nextpage.focuspic         = GAMESERVER_ARROW_RIGHT_A;
	...
	s_startserver.mapname.color         = color_grey;

	s_startserver.back.generic.type	    = MTYPE_BITMAP;
	s_startserver.back.generic.name     = GAMESERVER_BACK;
	...
	s_startserver.back.generic.y		= 425;
	s_startserver.back.width  		    = 96;
	s_startserver.back.height  		    = 32;
	s_startserver.back.focuspic         = GAMESERVER_BACK_A;

	s_startserver.next.generic.type	    = MTYPE_BITMAP;
	s_startserver.next.generic.name     = GAMESERVER_NEXT;
	...
	s_startserver.next.generic.x		= 522;
	s_startserver.next.generic.y		= 425;
	s_startserver.next.width  		    = 96;
	s_startserver.next.height  		    = 32;
	s_startserver.next.focuspic         = GAMESERVER_NEXT_A;
...
void StartServer_Cache( void )
{
	...
	trap_R_RegisterShaderNoMip( GAMESERVER_LOGO );	
	trap_R_RegisterShaderNoMip( GAMESERVER_BACK );	
	trap_R_RegisterShaderNoMip( GAMESERVER_BACK_A );	
	trap_R_RegisterShaderNoMip( GAMESERVER_NEXT );	
	trap_R_RegisterShaderNoMip( GAMESERVER_NEXT_A );	
	...
	trap_R_RegisterShaderNoMip( GAMESERVER_ARROW_LEFT );
	trap_R_RegisterShaderNoMip( GAMESERVER_ARROW_LEFT_A );
	trap_R_RegisterShaderNoMip( GAMESERVER_ARROW_RIGHT );
	trap_R_RegisterShaderNoMip( GAMESERVER_ARROW_RIGHT_A );
...
static void ServerOptions_LevelshotDraw( void *self ) {
	...
	x += b->width / 2;
	y += 4;
	UI_DrawString( x, y, s_serveroptions.mapnamebuffer, UI_CENTER|UI_SMALLFONT, color_grey );

	y += SMALLCHAR_HEIGHT;
	UI_DrawString( x, y, gametype_items[gametype_remap2[s_serveroptions.gametype]], UI_CENTER|UI_SMALLFONT, color_grey );
}
...
static void PlayerName_Draw( void *item ) {
	...
	{
		color = color_red;//text_color_highlight;
		style |= UI_PULSE;
	}
	else if ( s->generic.flags & QMF_BLINK )
	{
		...
		color = color_red;//text_color_normal;
...
static void ServerOptions_MenuInit( qboolean multiplayer ) {
	...
	s_serveroptions.player0.color					= color_red;
	...
	for( n = 0; n < PLAYER_SLOTS; n++ ) {
		...
		s_serveroptions.playerName[n].color				= color_red;
	...
	s_serveroptions.back.generic.name     = GAMESERVER_BACK;
	...
	s_serveroptions.back.generic.y		  = 425;
	s_serveroptions.back.width  		  = 96;
	s_serveroptions.back.height  		  = 32;
	s_serveroptions.back.focuspic         = GAMESERVER_BACK_A;

	s_serveroptions.next.generic.type	  = MTYPE_BITMAP;
	s_serveroptions.next.generic.name     = GAMESERVER_NEXT;
	...
	s_serveroptions.next.generic.x		  = 522;
	s_serveroptions.next.generic.y		  = 425;
	s_serveroptions.next.generic.statusbar  = ServerOptions_StatusBar;
	s_serveroptions.next.width  		  = 96;
	s_serveroptions.next.height  		  = 32;
	s_serveroptions.next.focuspic         = GAMESERVER_NEXT_A;

	s_serveroptions.go.generic.type	    = MTYPE_BITMAP;
	s_serveroptions.go.generic.name     = GAMESERVER_FIGHT;
	...
	s_serveroptions.go.generic.x		= 522;
	s_serveroptions.go.generic.y		= 425;
	s_serveroptions.go.width  		    = 96;
	s_serveroptions.go.height  		    = 32;
	s_serveroptions.go.focuspic         = GAMESERVER_FIGHT_A;
...
void ServerOptions_Cache( void ) {
	...
	trap_R_RegisterShaderNoMip( GAMESERVER_BACK );
	trap_R_RegisterShaderNoMip( GAMESERVER_BACK_A );
	trap_R_RegisterShaderNoMip( GAMESERVER_FIGHT );
	trap_R_RegisterShaderNoMip( GAMESERVER_FIGHT_A );
	trap_R_RegisterShaderNoMip( GAMESERVER_SELECT );
	trap_R_RegisterShaderNoMip( GAMESERVER_UNKNOWNMAP );
}
...
static void UI_BotSelectMenu_Init( char *bot ) {
	...
	for( i = 0, k = 0; i < PLAYERGRID_ROWS; i++) {
		...
		for( j = 0; j < PLAYERGRID_COLS; j++, k++ ) {
			...
			botSelectInfo.pics[k].focuscolor				= color_red;
			...
			botSelectInfo.picbuttons[k].focuscolor			= color_red;
			...
			botSelectInfo.picnames[k].color					= color_red;
