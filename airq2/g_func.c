...
void plat_blocked (edict_t *self, edict_t *other)
{
	...
	if ( !other->owner->client )	// +BD +owner
	{
...
RUNWAYS
  ...
  just used for takeoff and landing to check for
  the classname
...
Every place a plane/helo can land
on should be a func_runway
*/

void SP_func_runway (edict_t *ent)
{
	ent->movetype = MOVETYPE_NONE;
	ent->solid  = SOLID_BBOX;
	ent->health = DAMAGE_NO; 		// will be changed later so that
									// enemy can destroy runways
	ent->classname = "func_runway";
	gi.setmodel( ent, ent->model );
	gi.linkentity( ent );
}
