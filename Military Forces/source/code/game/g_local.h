...
typedef struct scripttask_s scripttask_t;

struct scripttask_s {
	taskTypes_t			type;						// type of task
	gentity_t*			owner;						// which drone ownes this task
...
bool G_RadiusDamage (vec3_t origin, gentity_t *attacker, float damage, float radius, gentity_t *ignore, int mod, long cat);
...
// g_mfq3ents.c
//
void radio_comply( gentity_t *ent );
